...
fn undo_line(numbers: bool, re: &regex::Regex, next_offset: u64, line: &str) -> Result<Vec<u8>, String> {
    ...
    if numbers {
        let offset = try!(u64::from_str_radix(&cap[1], 16).map_err(|e|
            format!("offset looked like hex, but was rejected: '{}': {}",
                    &cap[1], e)));

        if offset != next_offset {
            return Err(format!("invalid offset, expected {} but was {}",
                                next_offset, offset));
        }
    }
    ...
    let bytes: Vec<u8> = try!(data.from_hex().map_err(|e|
                    format!("data ({}) looked like hex, but was rejected: {}", data, e)));
    ...
    for (line_no, line_dumb) in stdin.lock().lines().enumerate() {
        ...
        match undo_line(numbers, &re, next_offset, line.as_str()) {
            Ok(bytes) => {
                ...
                try!(dest.write(bytes.as_slice()).map_err(|e| {
                    format!("writing output failed: {}", e)
                }));
            },
            Err(msg) => return Err(format!("error: {} on line {}: {}",
                                    msg, line_no, line))
        };
    }
    return Ok(());
...
fn main() {
    ...
    let mut numbers = true;
    {
        ...
        ap.refer(&mut numbers)
            .add_option(&["-n", "--no-addresses"], StoreFalse,
